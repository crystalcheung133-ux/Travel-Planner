...
const tripData = window.CCMV_TRIP_DATA || { planId:"ccmv-saigon-2026", title:"CCMV Journey Planner", subtitle:"Vietnam 2026", reviewers:[{id:"Christal",name:"Christal",emoji:"🧸"},{id:"Mero",name:"Mero",emoji:"✝️"},{id:"Vivian",name:"Vivian",emoji:"👟"}] };
...
async function deleteSupabaseReview(kind,id){
  if(!supabaseReady || !supabaseClient) return false;
  const item_id=remoteItemId(kind,id);
  try{
    setSyncStatus('Clearing…','warn');
    const {error}=await supabaseClient
      .from(CCMV_SUPABASE.table)
      .delete()
      .eq('trip_id',CCMV_SUPABASE.planId)
      .eq('item_id',item_id)
      .eq('reviewer',currentReviewer);
    if(error) throw error;
    setSyncStatus('✓ Cleared from Supabase','online');
    return true;
  }catch(err){
    console.warn('CCMV Supabase clear failed:',err);
    setSyncStatus('Cleared locally · sync failed','warn');
    return false;
  }
}
...
const reviewers=(tripData.reviewers||[]).reduce((acc,r)=>{acc[r.id]=`${r.emoji} ${r.name}`;return acc;}, {Christal:'🧸 Christal',Mero:'✝️ Mero',Vivian:'👟 Vivian'});
const reviewerNames=Object.keys(reviewers);
let currentReviewer=localStorage.getItem('ccmv:reviewer')||'Christal';
...
function ensureCommentControls(box,kind='activity'){
  ...
  if(!actions) return;
  let save=actions.querySelector(kind==='activity'?'[data-save]':kind==='wrap'?'[data-wrap-save]':kind==='planner'?'[data-planner-save]':'[data-overall-save]');
  if(!save){ save=document.createElement('button'); save.textContent='Save'; save.setAttribute(kind==='activity'?'data-save':kind==='wrap'?'data-wrap-save':kind==='planner'?'data-planner-save':'data-overall-save',''); actions.prepend(save); }
  ...
  if(!actions.querySelector('[data-status]') && !actions.querySelector('[data-wrap-note]') && !actions.querySelector('[data-overall-note]') && !actions.querySelector('[data-planner-note]')){ const s=document.createElement('span'); s.setAttribute('data-status',''); actions.appendChild(s); }
}
function setupCommentBox(box,{saved,editedAt,saveSelector,statusSelector,placeholderSaved,syncNote}){
  ensureCommentControls(box, box.closest('[data-day-wrap]')?'wrap':box.closest('[data-overall-review]')?'overall':box.closest('[data-planner-notes]')?'planner':'activity');
  ...
  if(hasSavedRecord && !isEditing){
    ...
    if(clear) clear.hidden=false;
    if(status) status.textContent=syncNote || (hasSavedRecord?'Editing saved review':'Not saved yet');
  }
}
...
function setReviewer(name){
  currentReviewer=reviewers[name]?name:'Christal';
  localStorage.setItem('ccmv:reviewer',currentReviewer);
  document.querySelectorAll('.comment-box,[data-day-wrap],[data-overall-review],[data-planner-notes]').forEach(b=>delete b.dataset.editing);
  renderAll();
}
...
function renderActivity(card){
  ...
  card.classList.toggle('review-open', !!(hasSavedActivity(id) || (box && box.dataset.editing==='true')));
  ...
  badge.textContent=keepCount===reviewerNames.length?`✅ All reviewers Keep · ${reviewerNames.length} / ${reviewerNames.length}`:`${reviewedCount} / ${reviewerNames.length} reviewed · ${keepCount} Keep`;
...
function renderDaySummary(container){
  ...
  const dayId=container.dataset.daySummary || section.id;
  const wrapId=`${dayId}-wrap`;
  const plannerId=`${dayId}-planner-notes`;
  if(!cards.length){ container.innerHTML=''; return; }
  const overallCounts={keep:0,discuss:0,replace:0,none:0};
  cards.forEach(card=>{
    reviewerNames.forEach(name=>{
      const r=localStorage.getItem(reviewKey('reaction',safeCardKey(card),name))||'none';
      overallCounts[r]=(overallCounts[r]||0)+1;
    });
  });
  const activityRows=cards.map(card=>{
    const id=safeCardKey(card);
    ...
    const comments=reviewerNames.map(name=>{
      ...
      const r=localStorage.getItem(reviewKey('reaction',id,name));
      return `<div class="day-summary-comment-item"><b>${reviewers[name]} · ${reactionIcon(r)} ${reactionLabel(r)}</b>${esc(cm)}</div>`;
    }).filter(Boolean).join('');
...
${commentCount?`<details class="day-summary-comments"><summary>${commentCount} comment${commentCount>1?'s':''}</summary>${comments}</details>`:`<div class="summary-empty">No comments yet.</div>`}
</div>`;
  }).join('');
  const wrapRows=reviewerNames.map(name=>{
    const st=localStorage.getItem(reviewKey('wrap-status',wrapId,name));
    const cm=localStorage.getItem(reviewKey('wrap-comment',wrapId,name));
    if(!st && !cm) return '';
    return `<div class="day-summary-comment-item"><b>${reviewers[name]} ${st?`· ${reactionIcon(st)} ${reactionLabel(st)}`:'· Comment'}</b>${cm?esc(cm):'<span class="muted">No written comment</span>'}</div>`;
  }).filter(Boolean).join('');
  const plannerNote=localStorage.getItem(reviewKey('planner-note',plannerId,'Planner'))||'';
  const plannerTime=localStorage.getItem(reviewKey('planner-note-edited',plannerId,'Planner'))||'';
  const wrapStatus=computeWrapStatus(wrapId);
  const dayName=(dayId||'day').replace('day','Day ');
  container.innerHTML=`<details class="day-summary-details">
<summary><span><b>${dayName} Summary</b><small>Open for Planner Notes + review results</small></span><span class="day-summary-status status-${wrapStatus}">${statusLabel[wrapStatus]}</span></summary>
<div class="planner-notes" data-planner-notes="${plannerId}">
  <p class="eyebrow">Planner Notes — Crystal to use</p>
  <h3>What to change / look into</h3>
  <textarea placeholder="例如：要改、要查、或者要再決定嘅位…"></textarea>
  <div class="comment-actions"><button data-planner-save>Save Planner Notes</button><button data-edit hidden>Edit</button><button data-clear hidden>Clear</button><span data-planner-note></span></div>
</div>
<div class="day-summary-divider"></div>
<div class="day-summary-overview"><span>🟢 ${overallCounts.keep||0} Keep</span><span>🟡 ${overallCounts.discuss||0} Discuss</span><span>🔴 ${overallCounts.replace||0} Replace</span><span>⚪ ${overallCounts.none||0} Not reviewed</span></div>
<div class="day-summary-table">${activityRows}</div>
<div class="day-summary-row"><div class="day-summary-row-head"><b>Day Wrap-up results</b><span class="day-summary-status status-${wrapStatus}">${statusLabel[wrapStatus]}</span></div>${wrapRows||'<div class="summary-empty">No Day Wrap-up submitted yet.</div>'}</div>
</details>`;
}
...
function renderOverallSummary(){
  ...
  const rows=reviewerNames.map(name=>{
    ...
    if(!cm) return '';
    return `<div class="summary-row"><div class="summary-row-head"><b>${reviewers[name]}</b><span class="summary-chip">Overall Review</span></div><div class="summary-comments"><div class="summary-comment">${esc(cm)}</div></div></div>`;
  }).filter(Boolean).join('');
...
function renderOverall(){
  ...
  if(thread){
    const items=reviewerNames.map(name=>{
      ...
      if(!cm) return '';
      return `<div class="thread-item"><b>${reviewers[name]} · Overall Review</b><p>${esc(cm)}</p>${time?`<p><small>Last edited ${time}</small></p>`:''}</div>`;
    }).join('');
    thread.innerHTML='<h3 class="overall-saved-title">Saved Overall Comments</h3>'+(items||'<div class="thread-item"><b>No overall comments yet</b><p>選好 reviewer 後留下整體 comment。</p></div>');
  }
}

function relocateDaySummaries(){
  document.querySelectorAll('[data-day-summary]').forEach(summary=>{
    const section=summary.closest('section[data-page]');
    if(!section) return;
    const wrap=section.querySelector('[data-day-wrap]');
    if(wrap && wrap.nextElementSibling!==summary) wrap.insertAdjacentElement('afterend', summary);
  });
}
function renderAll(){relocateDaySummaries();setupCollapsibleWhy();renderReviewer();document.querySelectorAll('.activity').forEach(renderActivity);document.querySelectorAll('[data-day-wrap]').forEach(renderWrap);document.querySelectorAll('[data-day-summary]').forEach(renderDaySummary);renderDashboard();renderDecisionStatus();renderSummaryActivities();renderFinal();renderOverall();renderOverallSummary();renderPlannerNotes();renderPlannerSummary();}
...
document.addEventListener('click',async e=>{
  ...
  if(clear){
    ...
    if(activity){ const id=safeCardKey(activity); ['reaction','comment','comment-edited'].forEach(t=>localStorage.removeItem(reviewKey(t,id))); localStorage.removeItem(pendingKey('reaction',id)); setSyncNote('activity',id,''); await deleteSupabaseReview('activity',id); const b=activity.querySelector('.comment-box'); if(b) delete b.dataset.editing; }
